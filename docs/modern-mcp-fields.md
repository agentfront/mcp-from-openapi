...
Extension-supplied `meta` is emitted **even when `emitMeta` is off**, merged key-by-key with `x-frontmcp.meta` winning over `x-mcp.meta`, and both winning over the generated entry. `x-speakeasy-mcp` does not participate (outside its published contract).
