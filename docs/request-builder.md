...
| Headers/cookies | `simple` serialization, folded `Cookie` header, RFC 6265 name validation                                                       |
...
| Security        | Values applied only when present in `input`, with scheme-aware formatting (`Bearer` prefixes); missing credentials never throw |
| Safety          | Header-injection guards (CR/LF), path traversal-safe encoding, http(s)-only base URLs, unresolved-placeholder detection        |
...
Security mapper entries are applied only when a value is present in `input` — pass resolved credentials under the mapper's `inputKey` (e.g. from [SecurityResolver](./security.md)) or leave them out and inject auth in your transport layer. `http`/`oauth2` schemes get `Bearer `/`Basic ` prefixes automatically (never doubled).
